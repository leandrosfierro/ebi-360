...
export default async function SuperAdminDashboard() {
    try {
        ...
        const getSurveysCount = async () => {
            const { count, error } = await supabase.from("surveys").select("*", { count: "exact", head: true });
            if (error) console.error("Error fetching surveys count:", error);
            return count || 0;
        };
        ...
        return (
            <div className="p-8 text-center glass-card rounded-2xl border border-rose-500/20">
                ...
                <p className="text-muted-foreground">Ocurrió un error inesperado. Por favor intenta recargar la página.</p>
                {process.env.NODE_ENV !== 'production' && (
                    <pre className="mt-4 p-4 bg-black/50 text-xs text-left overflow-auto rounded text-rose-400">
